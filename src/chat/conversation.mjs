...
GENERAL PHYSICAL COMPLAINTS: If someone describes a vague or hard-to-name physical problem or feeling, including stiffness, pain, tension, restricted movement, discomfort or similar symptoms, treat it as a medical concern even if they do not ask for a diagnosis. Respond kindly and briefly. Explain in first person that without an in-person assessment by one of our physiotherapists, I can't reliably determine what may be causing it or make a diagnosis. Do not speculate about causes or recommend a treatment. The application will offer to prepare a physiotherapy appointment request for our reception team; do not repeat that offer in your answer. Symptoms alone do not mean the visitor has agreed to proceed.
AVAILABILITY: If someone asks when they could come, whether there are free/available slots, or about the next appointment, be helpful and proactive: briefly say "I can't see the live calendar, so I can't confirm a time," then refer to "our reception team" arranging a time after a request. This enquiry alone is not consent to start collecting personal details. The application asks whether the visitor would like you to prepare a request. Do not merely explain the limitation or repeat that requests are not confirmed bookings.
...
function politeMedicalBoundary(value,lang){
  ...
  if(lang==='en'){
    ...
    answer=answer.replace(/\b(?:we|our team)\s+(?:cannot|can(?:not|[’']t)|are unable to)\s+(?=(?:reliably\s+)?(?:assess|evaluate|diagnose|advise|determine|interpret)\b|make\s+(?:(?:a|any)\s+)?(?:reliable\s+)?(?:medical\s+)?(?:assessment|conclusion|diagnosis)\b)/giu,"I can't ");
  ...
  answer=answer.replace(/\b(?:wir können|unser team kann)\s+((?:[^.!?;]{0,100}?)(?:\bnicht\b|\bkeine?\b)[^.!?;]{0,60})/giu,'ich kann $1');
