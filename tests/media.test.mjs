...
test('source migration retains original medical paragraphs and dated reimbursement amounts',()=>{
  ...
  for(const entry of original){const migrated=content[entry.collection].find(r=>r.id===entry.id);assert.ok(migrated,entry.id);for(const key of ['intro','body'])if(entry.data[key])assert.equal(migrated[key],entry.data[key]);}
  assert.equal(content.services.find(r=>r.id==='crafta').body.includes('Hirnnervenprobleme'),true);
  assert.equal(content.reimbursements.length,7);assert.equal(content.reimbursements[0].asOf,'04/2023');assert.equal(content.prices[0].amount,'250');
  assert.equal(content.team.length,12);assert.equal(content.team.filter(t=>t.fictional).length,11);assert.equal(content.team[0].title,'Anna Katharina Plank, BSc');db.close();
});
