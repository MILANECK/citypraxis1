...
test('source migration retains original medical paragraphs and dated reimbursement amounts',()=>{
  ...
  for(const entry of original.filter(entry=>entry.collection!=='prices')){const migrated=content[entry.collection].find(r=>r.id===entry.id);assert.ok(migrated,entry.id);for(const key of ['intro','body'])if(entry.data[key]&&!(entry.id==='about'&&key==='intro'))assert.equal(migrated[key],entry.data[key]);}
