export function showAIUsage(panel,data,language='de'){
  ...
  for(const [label,value] of [
    ...
    [t('Monthly AI budget','Monatsbudget KI'),money(data.budget)],
    [t('Budget remaining','Verbleibendes Budget'),costAvailable?money(data.remaining):'—'],
    [t('Chat conversations this month','Chat-Gespräche diesen Monat'),String(Number(data.conversations)||0)]
  ]){
  ...
  if(costAvailable&&data.budget!==null){
  ...
  if(!costAvailable){
    ...
    if(data.budget===null)note(t('The monthly budget setting is invalid.','Die Einstellung für das Monatsbudget ist ungültig.'));
