...
function teamCardName(title){
  const full=String(title||'').trim(),comma=full.indexOf(','),name=comma<0?full:full.slice(0,comma),suffix=comma<0?'':full.slice(comma),breakAt=name.lastIndexOf(' ');
  if(breakAt<0)return esc(full);
  return `<span class="team-given-name">${esc(name.slice(0,breakAt))} </span><strong class="team-surname">${esc(name.slice(breakAt+1))}</strong>${esc(suffix)}`;
}
function teamCard(t){
  return `<article class="team-person"><a class="team-profile-link" href="${esc(teamPath(t))}" aria-labelledby="team-name-${esc(t.id)}"><div class="team-portrait">${t.image?`<img class="team-photo" src="${esc(optimizedImage(t.image))}" alt="" loading="lazy" decoding="async" width="360" height="360">`:'<div class="team-no-photo" aria-hidden="true">CP</div>'}</div><div class="team-card-copy">${t.id==='isabella-casny'?`<span class="team-lead-label">${I18n.language==='en'?'Practice director':'Praxisleitung'}</span>`:''}<h3 id="team-name-${esc(t.id)}">${teamCardName(t.title)}</h3><p class="team-role">${esc(t.role).replaceAll(' / ','<br>').replaceAll(' · ','<br>')}</p><span class="team-profile-prompt">${I18n.language==='en'?'View profile':'Profil ansehen'} ${arrow}</span></div></a></article>`;
}
