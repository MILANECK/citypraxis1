...
function therapistSelection(){
  ...
  if(!selected)return requested?'<p class="therapist-choice-note">'+(en?'This profile is no longer available. Our secretary will help you find suitable care.':'Dieses Profil ist nicht mehr verfügbar. Unser Sekretariat hilft Ihnen bei der passenden Betreuung.')+'</p>':'';
...
function contact() {
  ...
  const introduction=en?'Citypraxis is centrally located in Vienna’s 1st district, close to Palais Coburg, the Marriott, Wollzeile, Stephansplatz and Stadtpark.':'Die Citypraxis liegt zentral im 1. Wiener Gemeindebezirk – nahe Palais Coburg, dem Marriott, der Wollzeile, dem Stephansplatz und dem Stadtpark.';
  const practiceDetails=en?`<p>Our welcoming period practice is on the mezzanine level of a historic Viennese townhouse, with lift access and four treatment rooms. Herringbone solid-wood floors and traditional box windows give the rooms their character. Tea and coffee are available, and there is a shower if you need one before treatment.</p><h3>Getting here</h3><p>The U3 Stubentor station is about a 3-minute walk away. Tram 2 and buses 74A and 3A also stop nearby. Stephansplatz (U1/U3) and Stadtpark (U4) are each about 600 metres away; Landstraße (U3/U4) is about 700 metres away.</p>`:`<p>Unsere einladende Altbaupraxis liegt im Mezzanin eines Wiener Bürgerhauses aus der Jahrhundertwende. Sie ist mit dem Lift erreichbar und verfügt über vier Therapieräume. Vollholz-Fischgrätparkett und Kastenfenster prägen die Räume. Tee und Kaffee stehen bereit; bei Bedarf können Sie vor der Behandlung auch duschen.</p><h3>So kommen Sie zu uns</h3><p>Die U3-Station Stubentor ist etwa 3 Gehminuten entfernt. In der Nähe halten außerdem die Straßenbahnlinie 2 sowie die Buslinien 74A und 3A. Stephansplatz (U1/U3) und Stadtpark (U4) liegen jeweils etwa 600 Meter entfernt, Landstraße (U3/U4) etwa 700 Meter.</p>`;
  return article('Mitten in Wien. Ganz bei Ihnen.',introduction,'',`<div class="contact-grid"><div class="info-card"><span class="eyebrow">${en?'CONTACT':'SO ERREICHEN SIE UNS'}</span><h2>${esc(s.address)}</h2><p>${esc(s.city)}</p><a href="tel:${esc(s.phone.replaceAll(' ',''))}">${esc(s.phone)}</a><a href="mailto:${esc(s.email)}">${esc(s.email)}</a><h3 id="oeffnungszeiten">${en?'Appointments & opening hours':'Termine & Öffnungszeiten'}</h3><p class="hours-intro">${esc(s.hours)}</p>${weeklyHours(s)}<a class="button" href="/termin">${en?'Request a first appointment':'Ersttermin anfragen'} ↗︎</a></div><section class="map-card" aria-label="${I18n.translate('Anfahrt zur Citypraxis')}"><div class="contact-map"><iframe title="${esc(I18n.translate('Google Maps: Citypraxis, Stubenbastei 12/11, 1010 Wien'))}" src="${mapEmbedUrl.replaceAll('&','&amp;')}" loading="lazy" referrerpolicy="no-referrer-when-downgrade" allowfullscreen></iframe><svg class="map-pin" viewBox="0 0 40 52" aria-hidden="true"><path fill="currentColor" d="M20 1C9.5 1 1 9.4 1 19.8 1 33.3 20 51 20 51s19-17.7 19-31.2C39 9.4 30.5 1 20 1Z"/><circle cx="20" cy="19" r="7" fill="white"/></svg></div><div class="faq-list map-details"><details><summary>${en?'About the practice & getting here':'Über die Praxis & Anfahrt'}<span aria-hidden="true">+</span></summary><div class="map-details-copy">${practiceDetails}<p class="map-details-address"><strong>${esc(s.address)} · ${esc(s.city)}</strong></p><a class="button button-outline" href="${esc(directions)}" target="_blank" rel="noopener noreferrer">${en?'Open directions':'Route planen'} ↗︎</a></div></details></div></section></div>`);
}
function route() {
...
function bind() {
  ...
  if(bookingForm){
    import('/booking-gradient.js?v=mobile-booking-balance-1').then(({initBookingGradient})=>initBookingGradient()).catch(()=>{});
