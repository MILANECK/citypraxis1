...
function appointment() {
  ...
  for(const item of fallback.slice(5,7))if(!concerns.some(existing=>existing.title===item.title||existing.titleEn===item.titleEn)){const index=concerns.findIndex(existing=>existing.custom===true);concerns.splice(index<0?concerns.length:index,0,item);}
