...
function route() {
  ...
  if(collection && parts[2]) {
    ...
    if(item) return collection==='services'?servicePage(item):article(item.title,item.intro,item.body,`<a class="button" href="/termin">Ersttermin anfragen ↗</a><a class="text-link article-link" href="/leistungen/${esc(item.service||'physiotherapie')}">Zur Behandlung →</a>`);
  }
...
async function boot(){
  try{const preview=new URLSearchParams(location.search).has('preview');const response=await fetch(preview?'/api/admin/content':'/api/content');if(!response.ok)throw new Error(preview?'Für die Entwurfsvorschau bitte als Editor anmelden.':'Inhalte konnten nicht geladen werden.');data=I18n.localizeContent(await response.json());$('#app').innerHTML=(preview?'<div class="preview-banner">Entwurfsvorschau · Änderungen sind noch nicht öffentlich. <a href="/admin">Zur Verwaltung ↗</a></div>':'')+header()+`<main id="main">${route()}</main>`+footer();I18n.apply();const title=$('h1')?.textContent;document.title=(title?`${title} · `:'')+'Citypraxis Wien';bind();}
