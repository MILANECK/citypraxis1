...
function teamThumbnail(t){
  return `<a class="team-thumbnail" href="/ueber-uns" aria-label="${esc(t.title)} – Team ansehen${t.placeholder||t.fictional?' (Beispielprofil)':''}">${t.image?`<img src="${esc(optimizedImage(t.image))}" alt="" loading="lazy" decoding="async">`:'<span class="thumbnail-empty" aria-hidden="true"></span>'}</a>`;
}
...
function teamCard(t){
  return `<article class="team-person"><div class="team-portrait">${t.image?`<img class="team-photo" src="${esc(optimizedImage(t.image))}" alt="${esc(t.title)}" loading="lazy" decoding="async">`:'<div class="team-no-photo" aria-label="Noch kein Portrait hinterlegt">CP</div>'}</div><div class="team-card-copy"><h3>${esc(t.title)}</h3><p class="team-role">${esc(t.role).replaceAll(' / ','<br>').replaceAll(' · ','<br>')}</p>${t.phone||t.email?`<div class="team-contact">${t.phone?`<a href="tel:${esc(t.phone.replaceAll(' ',''))}">${esc(t.phone)}</a>`:''}${t.email?`<a href="mailto:${esc(t.email)}">${esc(t.email)}</a>`:''}</div>`:''}</div></article>`;
}
